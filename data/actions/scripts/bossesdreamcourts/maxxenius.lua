		...
		if item.itemid == 10029 then
			if player:getPosition().x == pushPos.x and player:getPosition().y == pushPos.y and player:getPosition().z == pushPos.z then
				...
				if doCheckArea() == false then
					...
					playerTile = Tile(config.playerPositions[i]):getTopCreature()
					if playerTile:getStorageValue(14554) < os.time() then
						players = storePlayers[i]
						config.playerPositions[i]:sendMagicEffect(CONST_ME_POFF)
						players:teleportTo(config.newPos)
						players:setStorageValue(14554, os.time() + 20*60*60)
					else
					player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You need to wait a while, you recently challenge the Maxxenius.")
					return true
					end
				end
					Position(config.newPos):sendMagicEffect(11)
					areaMaxxenius = addEvent(clearArea, 15 * 60000)

					Game.createMonster("Maxxenius", {x = 32207, y = 32047, z = 14}, false, true)
